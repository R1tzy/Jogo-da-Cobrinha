...
class Apple:
    ...
    def move(self):
        # fiz a divisão de SIZE(pixel) pelo tamanho da tela
...
class Snake:
    def __init__(self, parent_screen, length):
        self.parent_screen = parent_screen
        self.block = pygame.image.load("img/block.png").convert()
        self.direction = "down"
        # array vazio multiplicado pelo tamanho
        # ele coloca no array a quantidade do tamanho (ex.:[67]*3 = [67,67,67])
...
class Game:
    ...
    def play(self):
        ...
        if self.is_collision(self.snake.x[0], self.snake.y[0], self.apple.x, self.apple.y):
            self.play_sound('hit-apple')
            self.snake.increase_lenght()
            self.apple.move()
            
        #colisão da cobra consigo mesma
